...
RUN apt install -y software-properties-common curl git python3.12-dev cmake libmariadb-dev-compat libluajit-5.1-dev libzmq3-dev zlib1g-dev libssl-dev binutils-dev gcc-14 g++-14 && apt clean all
...
&& if [ $(uname -p) = "aarch64" ]; sed -i '/for (const char& c : name)/,/return false;/c\    for (const char& c : name) {\n        if (!std::isprint(static_cast<unsigned char>(c))) {\n            return false;\n        }\n    }' src/map/lua/luautils.cpp ; fi \
